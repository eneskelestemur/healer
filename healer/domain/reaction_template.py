...
class ReactionTemplate21:
    ...
    def from_reaction_json(cls, name: str, reaction_json: dict):
        ...
        valid_params = {
            key: val for key, val in reaction_json.items() if key in cls_parameters
        }
    ...
    def _sanitize_reaction(self):
        ...
        try:
            ...
            n_reactants, n_products = (
                len(self.get_reactants()),
                len(self.get_products()),
            )
            ...
            logger.warning(
                "Reaction %r failed to sanitize and will be skipped: %s", self.name, e
            )
    ...
    def get_reactants(self, sort_by_mw=True):
        ...
        return sorted(
            self._reaction.GetReactants(),
            key=lambda x: Descriptors.MolWt(x),
            reverse=True,
        )
    ...
    def get_reactant_index(self, mol: Chem.Mol):
        ...
        return [
            i for i, reactant in enumerate(reactants) if mol.HasSubstructMatch(reactant)
        ]
    ...
    def _order_reactants_by_annotations(self, reactants):
        ...
        """
        ann0 = (
            reactants[0].get_parsed_prop("rxn_annotations").get(self.name, [])
            if isinstance(reactants[0], BuildingBlock)
            else self.get_reactant_index(reactants[0])
        )
        ann1 = (
            reactants[1].get_parsed_prop("rxn_annotations").get(self.name, [])
            if isinstance(reactants[1], BuildingBlock)
            else self.get_reactant_index(reactants[1])
        )
