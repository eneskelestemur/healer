...
from rdkit.Chem import QED, Descriptors, rdDepictor
    ...
    if USE_CELERY:
        ...
        if job_id in _local_jobs:
            ...
            return {
                "job_id": job_id,
                "status": "cancelled",
                "note": "Local mode - job may have already completed",
            }
...
async def get_server_limits_endpoint():
    ...
    return {"server_mode": SERVER_MODE, "limits": get_server_limits()}
...
async def download_job_results(job_id: str):
    ...
    response.headers["Content-Disposition"] = (
        f"attachment; filename=healer_results_{job_id}.csv"
    )
    return response
...
async def smiles_to_molfile(request: SmilesRequest):
    try:
        ...
        raise HTTPException(status_code=500, detail="Error processing molecule") from e
...
async def render_mol_with_indices(request: SmilesRequest):
    ...
    try:
        ...
        svg_data_uri = utils.get_svg_mol(
            mol, legend="", show_idx=True, width=250, height=125
        )
        ...
        raise HTTPException(status_code=500, detail="Error rendering molecule") from e
...
async def render_result(request: RenderRequest):
    ...
    try:
        ...
        bg_color = (
            request.bgColor.replace(" ", "")
            .replace("rgba(", "")
            .replace(")", "")
            .split(",")
        )
        bg_color = tuple(
            int(c) / 255.0 for c in bg_color[:3]
        )  # ignore alpha for bg color
        if request.bbs:
            ...
            if valid_bbs:
                try:
                    svg_data_uri = utils.get_svg_mol_with_bbs(
                        ...
                        legend="",
                        alpha=alpha,
                        bg_color_for_transparency=bg_color,
                    )
        ...
        try:
            ...
            logger.error(
                "Error rendering result: %s (fallback render also failed: %s)",
                e,
                fallback_error,
            )
            raise HTTPException(status_code=500, detail="Error rendering result") from e
