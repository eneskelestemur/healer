...
def plot_retrosynthesis_tree(
    ...
    ] = {}  # maps objects to (x, y) positions

    def compute_positions(obj, depth=0) -> int:
        from healer.application.tree_builder import SplitBranch, SplitNode
        ...
        if isinstance(obj, SplitNode):
            ...
            if name_prop == "image":
                ...
                if name_prop == "smiles":
                    ...
                    label = (
                        obj.molecule.GetProp(name_prop)
                        if obj.molecule.HasProp(name_prop)
                        else f"node_{id(obj) % 1000}"
                    )
                ax.text(
                    x,
                    y,
                    ...
                    va="center",
                    fontsize=font_size,
                    bbox={"boxstyle": "round", "fc": "white", "ec": "black", "lw": 0.5},
                )
        else:
