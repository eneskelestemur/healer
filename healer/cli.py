...
def get_init_kwargs(args: argparse.Namespace, healer_type: str) -> dict:
    ...
    kwargs = {
        ...
        "reaction_tags": args.reactions.split(",")
    ...
    if healer_type == "site":
        ...
        kwargs["struct_rules"] = (
            args.struct_rules.split(",") if args.struct_rules else []
        )
...
def run_enumeration(
    ...
    *enumerate_kwargs* and handled by joblib inside the synthesis loop.
    ...
    with progress_bar(
        smiles_list, desc="Enumerating", unit="mol", show_progress=show_progress
    ) as bar:
        for i, smiles in enumerate(bar, start=1):
            ...
            try:
                ...
                df.to_csv(
                    str(out), mode="w" if first else "a", header=first, index=False
                )
                first = False
                n_written += len(df)
...
def add_common_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "-o",
        "--output",
        default="healer_results.csv",
        help="Output CSV path (default: healer_results.csv)",
    )
    parser.add_argument(
        "--column", default="smiles", help="SMILES column name in CSV (default: smiles)"
    )
    ...
    parser.add_argument(
        "--shuffle", action="store_true", help="Shuffle building block order"
    )
    ...
    parser.add_argument(
        "--properties", action="store_true", help="Calculate molecular properties"
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    mol_parser = subparsers.add_parser(
        "molecule", help="Enumerate from whole molecules"
    )
    ...
    mol_parser.add_argument(
        "--randomize", action="store_true", help="Randomize composition order"
    )
    mol_parser.add_argument(
        "--seed",
        type=int,
    ...
    site_parser = subparsers.add_parser(
        "site", help="Enumerate at specific reactive sites"
    )
    ...
    frag_parser = subparsers.add_parser(
        "fragment", help="Enumerate from pre-split fragments"
    )
