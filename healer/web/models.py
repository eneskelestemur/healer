...
class MoleculeRequest(BaseModel):
    ...
    bb_source: str = Field(
        "test", description="Building block source (e.g., 'test', 'US_stock')"
    )
    ...
    sim_threshold: float = Field(
        0.15, description="Similarity threshold for BB selection"
    )
    n_compositions: int = Field(10, description="Number of compositions to generate")
    randomize_compositions: bool = Field(
        False, description="Whether to randomize composition order"
    )
    ...
    # New/Renamed fields
    max_bbs_per_frag: int = Field(
        -1, description="Max building blocks per fragment (-1 for unlimited)"
    )
    shuffle_bb_order: bool = Field(False, description="Shuffle building block order")

    # Limits
    max_evals_per_comp: Optional[int] = Field(
        None, description="Max reaction attempts per composition"
    )
    max_products_per_comp: Optional[int] = Field(
        None, description="Max products per composition"
    )
...
class SiteRequest(BaseModel):
    ...
    reactive_sites: Optional[List[int]] = Field(
        None, description="List of reactive atom indices"
    )
    ...
    struct_rules: Optional[List[str]] = Field(
        default=[], description="SMARTS patterns for structure rules"
    )
