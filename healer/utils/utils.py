...
def make_rgb_transparent(
    rgb: Tuple[int, int, int], bg_rgb: Tuple[int, int, int], alpha: float
) -> Tuple[int, int, int]:
    """
    ...
    """
    return tuple(
        alpha * c1 + (1 - alpha) * c2 for (c1, c2) in zip(rgb, bg_rgb, strict=True)
    )
...
def get_batch_tversky_sims(
    ...
    for i, query_fp in enumerate(query_fps):
        sims[i] = DataStructs.BulkTverskySimilarity(
            query_fp, stock_fps, a=query_factor, b=stock_factor
        )
    return sims
...
def get_svg_mol(
    ...
    drawing.DrawMolecule(
        ...
        highlightAtomColors=dict.fromkeys(hit_atoms, sub_mol_color),
        highlightBondColors=dict.fromkeys(hit_bonds, sub_mol_color),
        legend=legend,
    )
...
def get_svg_mol_with_bbs(
    ...
    if bb_colors is None:
        ...
        bb_colors = [
            rdColors[rd_color_names[i % len(rd_color_names)]] for i in range(num_bbs)
        ]
    ...
    if alpha < 1.0:
        bb_colors = [
            make_rgb_transparent(c, bg_color_for_transparency, alpha) for c in bb_colors
        ]

    if num_bbs != len(bb_colors):
        raise ValueError(
            f"Number of building blocks ({num_bbs}) does not match number of colors ({len(bb_colors)})"
        )
    ...
    for i, bb in enumerate(new_bbs):
        ...
        for bond in bb.GetBonds():
            ...
            bond_between = mol.GetBondBetweenAtoms(a1, a2)
            if bond_between is not None:
                hit_bonds.append(bond_between.GetIdx())
    ...
    drawing.DrawMolecule(
        mol,
        highlightAtoms=list(chain.from_iterable(hit_atoms_list)),
        highlightBonds=list(chain.from_iterable(hit_bonds_list)),
